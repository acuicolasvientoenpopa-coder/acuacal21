  ...
  for (const f of fincas) {
    for (const e of (f.estanques || [])) {
      est.push({
        id: `${f.id}||${e}`,
        label: `${f.nombre} → ${e}`,
        raw: { fincaId: f.id, fincaNombre: f.nombre, estanqueNombre: e },
      });
    }
  }
  return est;
...
export function useLookups() {
  const [species, setSpecies] = useState<LookupItem[]>(() => {
    try {
      ...
      return ESPECIES_DEFAULT.map((s) => ({ id: s.id, label: s.nombre, raw: s }));
    }
  });
  const [fincas, setFincas] = useState<LookupItem[]>(() => {
    try {
      return (JSON.parse(localStorage.getItem("aquacalc_fincas") || "[]")).map((f: any) => {
        const fm = migrarFinca(f);
        return { id: fm.id, label: fm.nombre, raw: fm };
      });
    } catch { return []; }
  });
  ...
  const reload = useCallback(() => {
    try {
      ...
      setSpecies(ESPECIES_DEFAULT.map((s) => ({ id: s.id, label: s.nombre, raw: s })));
    }
    try {
      const fs = (JSON.parse(localStorage.getItem("aquacalc_fincas") || "[]")).map(migrarFinca);
