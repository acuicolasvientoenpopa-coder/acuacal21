...
const speciesToForm = (s: Species): FormState => ({
  nombre: s.nombre,
  sci: s.sci,
  emoji: s.emoji,
  ...s.params,
...
export default function Especies() {
  ...
  const [custom, setCustom] = useState<Species[]>(loadCustom);
  const [show, setShow] = useState(false);
  const [edit, setEdit] = useState<Species | null>(null);
  const [form, setForm] = useState<FormState>(emptyForm);

  const saveToLS = useCallback((data: Species[]) => {
    localStorage.setItem(CUSTOM_KEY, JSON.stringify(data));
  }, []);

  useEffect(() => { saveToLS(custom); }, [custom, saveToLS]);

  useEffect(() => {
    const handler = (e: StorageEvent) => {
      if (e.key === CUSTOM_KEY) setCustom(loadCustom());
    };
  ...
  const save = () => {
    ...
    const s: Species = edit
      ? { ...edit, nombre: nombre.trim(), sci: sci.trim(), emoji, params }
      : { id: `custom_${Date.now()}`, nombre: nombre.trim(), sci: sci.trim(), emoji, custom: true, params };
    setCustom(edit ? custom.map((c) => (c.id === edit.id ? s : c)) : [...custom, s]);
    setShow(false);
  };
        ...
        <div className="modal-overlay" onClick={() => setShow(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="form-grid">
              <label>
                {t("especie")}
                <input value={form.nombre} onChange={(e) => setForm({ ...form, nombre: e.target.value })} placeholder="Ej: Tilapia Roja" />
              </label>
              <label>
                Científico
                <input value={form.sci} onChange={(e) => setForm({ ...form, sci: e.target.value })} placeholder="Ej: O. niloticus" />
              </label>
              <label>
                Emoji
            ...
            <div className="form-grid">
              <label>
                {t("densidadSiembra")}
                <input type="number" value={form.densidad} onChange={(e) => setP("densidad", e.target.value)} />
              </label>
              <label>
                Densidad unit
                <input value={form.densidadUnit} onChange={(e) => setForm({ ...form, densidadUnit: e.target.value })} placeholder="peces/m³" />
              </label>
              <label>
                {t("supervivencia")}
                <input type="number" value={form.supervivencia} onChange={(e) => setP("supervivencia", e.target.value)} />
              </label>
              <label>
                {t("fcr")}
                <input type="number" step="0.1" value={form.fcr} onChange={(e) => setP("fcr", e.target.value)} />
              </label>
              <label>
                {t("tasaAlim")}
                <input type="number" step="0.1" value={form.tasaAlim} onChange={(e) => setP("tasaAlim", e.target.value)} />
              </label>
              <label>
                {t("comidasDia")}
                <input type="number" value={form.comidasDia} onChange={(e) => setP("comidasDia", e.target.value)} />
              </label>
              <label>
                {t("gpd")}
                <input type="number" step="0.1" value={form.gpd} onChange={(e) => setP("gpd", e.target.value)} />
              </label>
              <label>
                {t("precioAlim")}
                <input type="number" value={form.precioAlimento} onChange={(e) => setP("precioAlimento", e.target.value)} />
              </label>
              <label>
                {t("precioVenta")}
                <input type="number" value={form.precioVenta} onChange={(e) => setP("precioVenta", e.target.value)} />
              </label>
              <label>
                {t("pesoInicial")}
                <input type="number" step="0.1" value={form.pesoInicial} onChange={(e) => setP("pesoInicial", e.target.value)} />
              </label>
              <label>
                {t("pesoCosecha")}
                <input type="number" value={form.pesoCosecha} onChange={(e) => setP("pesoCosecha", e.target.value)} />
              </label>
              <label>
                Volumen unit
                <input value={form.volumenUnit} onChange={(e) => setForm({ ...form, volumenUnit: e.target.value })} placeholder="m³" />
              </label>
            </div>
