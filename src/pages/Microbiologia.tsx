...
interface Cultivo {
  ...
  agente: string;
  carga: string;
  antibiograma: AntibiogramaEntry[];
...
function loadCultivos(): Cultivo[] {
  try { return JSON.parse(localStorage.getItem(CULTIVOS_KEY) || "[]"); }
  catch { return []; }
}

function saveCultivos(data: Cultivo[]) {
  localStorage.setItem(CULTIVOS_KEY, JSON.stringify(data));
}

function loadMedicacion(): Medicacion[] {
  try { return JSON.parse(localStorage.getItem(MEDICACION_KEY) || "[]"); }
  catch { return []; }
}

function saveMedicacion(data: Medicacion[]) {
  localStorage.setItem(MEDICACION_KEY, JSON.stringify(data));
}
...
function exportCultivoPDF(c: Cultivo) {
  const lines = [
    "AquaCalc - Reporte de Cultivo",
    "",
    "Fecha: " + c.fecha,
    "Estanque: " + c.estanqueNombre,
    "Especie: " + c.especie,
    "Muestra: " + c.tipoMuestra + (c.organo ? " - " + c.organo : ""),
    "Resultado: " + c.resultado.toUpperCase(),
    "Agente: " + (c.agente || "—"),
    "Carga: " + (c.carga || "—"),
    "",
    "--- Antibiograma ---",
  ...
  if (c.observaciones) {
    lines.push("", "Observaciones:", c.observaciones);
  }
  const blob = new Blob([lines.join("\r\n")], { type: "text/plain;charset=utf-8" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = "cultivo_" + c.id + ".txt";
  a.click();
...
export default function Microbiologia() {
  ...
  // Cultivos
  const [cultivos, setCultivos] = useState<Cultivo[]>(loadCultivos);
  ...
  // Medicacion
  const [medicacion, setMedicacion] = useState<Medicacion[]>(loadMedicacion);
  ...
  useEffect(() => {
    const h = (e: StorageEvent) => {
      if (e.key === CULTIVOS_KEY) setCultivos(loadCultivos());
      if (e.key === MEDICACION_KEY) setMedicacion(loadMedicacion());
    };
  ...
  const openCultivo = (c?: Cultivo) => {
    if (c) {
      setEditCultivo(c);
      setCf({ ...c, antibiograma: [...c.antibiograma] });
    } else {
      setEditCultivo(null);
      setCf(cultivoVacio());
    }
  ...
  const saveCultivo = () => {
    if (!cf.fecha || !cf.estanqueNombre || !cf.especie) {
      toast("Completá fecha, estanque y especie", "error");
      return;
    }
    const updated = editCultivo
      ? cultivos.map((c) => (c.id === editCultivo.id ? { ...cf, id: editCultivo.id } : c))
      : [...cultivos, { ...cf, id: "cult_" + Date.now() }];
    setCultivos(updated);
  ...
  const openMed = (m?: Medicacion) => {
    if (m) {
      ...
      setEditMed(null);
      setMf(medicacionVacia());
    }
    setShowMed(true);
  };

  const saveMed = () => {
    if (!mf.fechaInicio || !mf.estanqueNombre || !mf.producto) {
      toast("Completá fecha, estanque y producto", "error");
      return;
    }
    const updated = editMed
      ? medicacion.map((m) => (m.id === editMed.id ? { ...mf, id: editMed.id } : m))
      : [...medicacion, { ...mf, id: "med_" + Date.now() }];
    setMedicacion(updated);
    saveMedicacion(updated);
    setShowMed(false);
    toast(editMed ? "Medicación actualizada" : "Medicación creada", "success");
  };

  const deleteMed = (id: string) => {
    const updated = medicacion.filter((m) => m.id !== id);
    setMedicacion(updated);
    saveMedicacion(updated);
  ...
  const estadoColor = (e: string) =>
    e === "en_curso" ? "var(--accent3)" : e === "completado" ? "var(--accent)" : "var(--danger)";
  const estadoLabel = (e: string) =>
    e === "en_curso" ? t("microEnCurso") : e === "completado" ? t("microCompletado") : t("microSuspendido");
  ...
  return (
    <div>
      ...
      <div className="mini-tabs" style={{ marginBottom: 16 }}>
        <button className={"mini-tab" + (tab === "cultivos" ? " active" : "")} onClick={() => setTab("cultivos")}>
          🧫 {t("microCultivos")} ({cultivos.length})
        </button>
        <button className={"mini-tab" + (tab === "medicacion" ? " active" : "")} onClick={() => setTab("medicacion")}>
          💊 {t("microMedicacion")} ({medicacion.length})
        </button>
      </div>

      {/* Últimos tratamientos dashboard */}
      {medicacion.length > 0 && (
      ...
      {tab === "cultivos" && (
        <div>
          <button className="btn-primary btn-sm" style={{ marginBottom: 12 }} onClick={() => openCultivo()}>
            ＋ {t("microNuevoCultivo")}
          </button>
          {filtradosC.length === 0 ? (
                ...
                <div key={c.id} style={{ borderBottom: "1px solid var(--border)", padding: "12px 0" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div>
                      ...
                      <span className={"vet-tag " + (c.resultado === "positiva" ? "riesgo-rojo" : "riesgo-verde")} style={{ marginLeft: 8, fontSize: 11 }}>
                        {c.resultado === "positiva" ? t("microPositiva") : t("microNegativa")}
                      </span>
                    </div>
                  ...
                  {c.antibiograma.length > 0 && (
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginTop: 6 }}>
                      {c.antibiograma.filter((a) => a.sensibilidad === "R").slice(0, 3).map((a) => (
                        <span key={a.antibiotico} className="vet-tag" style={{ fontSize: 10, background: "rgba(220,60,50,0.15)", color: "var(--danger)" }}>
                          {a.antibiotico}: R
                        </span>
                      ))}
                    </div>
      ...
      {tab === "medicacion" && (
        <div>
          <button className="btn-primary btn-sm" style={{ marginBottom: 12 }} onClick={() => openMed()}>
            ＋ {t("microNuevaMedicacion")}
          </button>
          {filtradosM.length === 0 ? (
                  ...
                  <div key={m.id} style={{ borderBottom: "1px solid var(--border)", padding: "12px 0" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                      <div>
                        ...
                        <span className="vet-tag" style={{ fontSize: 11, background: estadoColor(m.estado) + "22", color: estadoColor(m.estado), border: "1px solid " + estadoColor(m.estado), marginLeft: 8 }}>
                          {estadoLabel(m.estado)}
                        </span>
                        {rest !== null && rest > 0 && (
                          <span className="vet-tag" style={{ fontSize: 11, background: "rgba(220,60,50,0.12)", color: "var(--danger)", marginLeft: 6 }}>
                            ⏳ {rest} {t("microDiasRestantes")}
                          </span>
                        )}
                      </div>
                      <div style={{ display: "flex", gap: 6 }}>
                    ...
                    <div style={{ fontSize: 11, color: "var(--text2)", marginTop: 4 }}>
                      {m.fechaInicio} → {m.fechaFin || "—"} | {m.via} | {m.dosis} | {t("microResponsable")}: {m.responsable || "—"}
                    </div>
                  </div>
                );
              })}
        ...
        <div className="modal-overlay" onClick={() => setShowCultivo(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3>{editCultivo ? t("microEditarCultivo") : t("microNuevoCultivo")}</h3>
              <button className="modal-close" onClick={() => setShowCultivo(false)}>✕</button>
            </div>
            <div className="form-grid">
              <label>{t("microFecha")}<input type="date" value={cf.fecha} onChange={(e) => setCf({ ...cf, fecha: e.target.value })} /></label>
              <label>{t("microEstanque")}
                <select value={cf.estanqueNombre} onChange={(e) => setCf({ ...cf, estanqueNombre: e.target.value })}>
                  <option value="">{t("seleccionar")}</option>
                  {estanques.map((e) => <option key={e.id} value={e.id}>{e.label}</option>)}
                </select>
              </label>
              <label>{t("microEspecie")}
                <select value={cf.especie} onChange={(e) => setCf({ ...cf, especie: e.target.value })}>
                  <option value="">{t("seleccionar")}</option>
                  {allSpecies.map((s) => <option key={s.id} value={s.id}>{s.label}</option>)}
                </select>
              </label>
              <label>{t("microTipoMuestra")}
                <select value={cf.tipoMuestra} onChange={(e) => setCf({ ...cf, tipoMuestra: e.target.value })}>
                  <option value="">—</option>
              ...
              <label>{t("microResultado")}
                <select value={cf.resultado} onChange={(e) => setCf({ ...cf, resultado: e.target.value as "positiva" | "negativa" })}>
                  <option value="positiva">{t("microPositiva")}</option>
                  <option value="negativa">{t("microNegativa")}</option>
                </select>
              </label>
            ...
            <div style={{ marginTop: 16 }}>
              ...
              <div style={{ overflowX: "auto" }}>
                <table className="zoo-table" style={{ fontSize: 12 }}>
                  <thead>
                    <tr>
                      <th>{t("microAntibiotico")}</th>
                      <th>{t("microSensible")}</th>
                      <th>{t("microIntermedio")}</th>
                      <th>{t("microResistente")}</th>
                    </tr>
                  </thead>
                  <tbody>
                        ...
                        {(["S", "I", "R"] as const).map((val) => (
                          <td key={val} style={{ textAlign: "center" }}>
                            <input
                              type="radio"
                              name={"ab_" + i}
                              checked={cf.antibiograma[i].sensibilidad === val}
                              onChange={() => {
                                const ab = [...cf.antibiograma];
                                ab[i] = { ...ab[i], sensibilidad: val };
                                setCf({ ...cf, antibiograma: ab });
                              }}
                            />
                          </td>
                        ))}
            ...
            <label style={{ marginTop: 12, display: "block" }}>
              {t("microObservaciones")}
              <textarea value={cf.observaciones} onChange={(e) => setCf({ ...cf, observaciones: e.target.value })} rows={2} />
            </label>
            <div className="modal-actions">
              <button className="btn-primary" onClick={saveCultivo}>{t("save")}</button>
              <button className="btn-secondary" onClick={() => setShowCultivo(false)}>{t("cancel")}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showMed && (
        <div className="modal-overlay" onClick={() => setShowMed(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3>{editMed ? "Editar" : t("microNuevaMedicacion")}</h3>
              <button className="modal-close" onClick={() => setShowMed(false)}>✕</button>
            </div>
            <div className="form-grid">
              <label>{t("microEstanque")}
                <select value={mf.estanqueNombre} onChange={(e) => setMf({ ...mf, estanqueNombre: e.target.value })}>
                  <option value="">{t("seleccionar")}</option>
                  {estanques.map((e) => <option key={e.id} value={e.id}>{e.label}</option>)}
                </select>
              </label>
              ...
              <label>{t("microVia")}
                <select value={mf.via} onChange={(e) => setMf({ ...mf, via: e.target.value })}>
                  <option value="">—</option>
              ...
              <label>{t("microEstado")}
                <select value={mf.estado} onChange={(e) => setMf({ ...mf, estado: e.target.value as "en_curso" | "completado" | "suspendido" })}>
                  <option value="en_curso">{t("microEnCurso")}</option>
                  <option value="completado">{t("microCompletado")}</option>
                  <option value="suspendido">{t("microSuspendido")}</option>
                </select>
              </label>
            </div>
            <div className="modal-actions">
              <button className="btn-primary" onClick={saveMed}>{t("save")}</button>
              <button className="btn-secondary" onClick={() => setShowMed(false)}>{t("cancel")}</button>
            </div>
          </div>
        </div>
      )}
